...
void Crawl::start(const string& url, const int& num_loaders,
  ...
  for (int i = 0; i < depth; ++i) {
    ...
    while (!all_levels[i].first.empty() || !all_levels[i].second.empty()) {
      if (!all_levels[i].first.empty()) {
        all_levels[i].second.push_back(
            downloader.load_html_list(all_levels[i].first.front()));
        all_levels[i].first.pop_front();
      }
      if (!all_levels[i].second.empty()) {
        for (auto& a :
             parser.parse(all_levels[i].second.front(), (i + 1 == depth))) {
...
Crawl::~Crawl() { print_img_to_file(path_out_file); }
