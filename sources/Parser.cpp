...
void Parser::search_for_links(GumboNode* node, const Html& html) {
  ...
  if (node->v.element.tag == GUMBO_TAG_A &&
    ...
    string link =
        (string{href->value}.find('/') == 0)
            ? string{html.protocol + "://" + html.host + string{href->value}}
            : href->value;
    if (all_links.empty()) {
      all_links.push_back(link);
    } else if (not_repeated_link(link, all_links) && link[0] == 'h' &&
               (link != string{html.protocol + "://" + html.host})) {
      all_links.push_back(link);
    }
  }
...
void Parser::search_for_img(GumboNode* node, const Html& html) {
  ...
  if ((node->v.element.tag == GUMBO_TAG_IMG ||
      ...
      string{img->value} != "") {
    string image =
        (string{img->value}.find('/') == 0)
            ? string{html.protocol + "://" + html.host + string{img->value}}
            : img->value;
...
std::deque<string> Parser::parse(const Html& html_, bool flag) {
  auto res = parsers.enqueue(
      [&](const Html& html) {
        if (!flag) find_links(html);
        find_img(html);
        return all_links;
      },
      html_);
